...
const Button = React.forwardRef((props: ButtonProps, ref: any) => {
  const { outline, size, color, children, fullWidth, ...rest } = props;
  ...
  className =
    className +
    " " +
    (colors[(color as string) + (outline ? "-outline" : "")] as string);
  className = className + " " + (sizes[size as string] as string);
  if (fullWidth) {
